...
func MapToRPCMessage(data map[string]interface{}) *RPCMessage {
    msg := &RPCMessage{}
    // Basic fields
    if val, ok := data["rpc"].(string); ok { msg.RPC = val }
    if val, ok := data["who"].(string); ok { msg.Who = val }
    if val, ok := data["message_id"].(string); ok { msg.MessageID = val }
    if val, ok := data["deadline"].(float64); ok { msg.Deadline = int64(val) }

    // Parse `args` and `stash` fields as nested JSON objects
    if args, ok := data["args"].(map[string]interface{}); ok { msg.Args = args }
    if stash, ok := data["stash"].(map[string]interface{}); ok { msg.Stash = stash }
